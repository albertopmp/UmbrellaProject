...
resource "aws_lambda_permission" "execute_subscribe_sns_topic" {
  ...
  principal     = "apigateway.amazonaws.com"
  source_arn    = "arn:aws:execute-api:${var.api_gw_region}:${var.api_gw_account_id}:${aws_api_gateway_rest_api.umbrella_api.id}/*/${aws_api_gateway_method.count_sns_topic_subs.http_method}${aws_api_gateway_resource.subscribers_id.path}"
}
